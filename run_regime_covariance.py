...
# Use cell-level cleaned data (recommended): only bad cells set to NaN, no days dropped.
# See data/docs/CELLWISE_CLEANING_REPORT.txt and data/docs/DATA_QUALITY_ISSUES.md.
# Alternatives: returns_universe_100_cleaned_moderate.parquet (drops 48 days), raw parquet.
USE_CLEANED_DATA = True
if USE_CLEANED_DATA:
    returns_df = pd.read_parquet("data/processed/returns_universe_100_cleaned_cellwise.parquet")
else:
