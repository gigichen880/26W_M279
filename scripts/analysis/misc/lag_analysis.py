...
def main():
    ...
    p_k1 = ablation_dir / "backtest_k1.parquet"
    p_k4 = ablation_dir / "backtest_k4.parquet"
    if not (p_k1.exists() and p_k4.exists()):
        print(f"(skip) Missing required files: {p_k1} and/or {p_k4}")
        return

    df_k1 = pd.read_parquet(p_k1)
    ...
    try:
        ...
        plt.tight_layout()
        outpath = resolve_figs_dir("regime_covariance") / "ablation_crisis_vs_normal.png"
