...
Supports both covariance and volatility backtests. Generates:
1) Model vs baselines (excluding mix)
2) Mix vs baselines (excluding model)

Backtest data is in wide format: one row per date,
columns like model_fro, roll_fro (cov) or model_vol_mse, roll_vol_mse (vol).

Usage:
  # Covariance (default)
  python scripts/analysis/visualize_statistical_comparison.py
  python scripts/analysis/visualize_statistical_comparison.py --input results/regime_covariance_backtest.parquet --target covariance
...
DEFAULT_BACKTEST_COV = RESULTS_DIR / "regime_covariance_backtest.parquet"
DEFAULT_BACKTEST_VOL = RESULTS_DIR / "regime_volatility_backtest.parquet"


# =============================================================================
# TARGET-SPECIFIC METRIC CONFIG
# =============================================================================
...
def _cov_config():
    return {
        ...
        "out_subdir": "figs_regime_covariance/statistical_comparison",
    }
...
def _vol_config():
    return {
        ...
        "out_subdir": "figs_regime_volatility/statistical_comparison",
    }


def get_target_config(target: str, df: pd.DataFrame):
    """Return metrics/labels/outdir for the given target. target 'auto' detects from columns."""
    if target == "auto":
...
def load_backtest_results(path=None):
    ...
    if path is None:
        for p in (DEFAULT_BACKTEST_COV, RESULTS_DIR / "regime_covariance_backtest.csv"):
            if p.exists():
                path = p
                break
        else:
...
def plot_statistical_comparison(
    ...
    *,
    ...
    results_df["normalized_diff"] = results_df.apply(
        lambda r: normalize_difference(r["metric"], r["mean_diff"], lower_is_better), axis=1
    )
...
def main():
    ...
    ap.add_argument("--target", default="auto", choices=("auto", "covariance", "volatility"),
                    help="Target type for metrics; auto = infer from columns")
    args = ap.parse_args()
    ...
    target_used = "volatility" if "model_vol_mse" in df.columns else "covariance"
    print(f"Target: {target_used} (metrics: {config['key_metrics_plot']})")
    ...
    model_results = compare_vs_reference(
        df, reference="model", baselines=("roll", "pers", "shrink"), metrics=metrics,
    )
    fig_path_model = out_dir / "model_vs_baselines.png"
    plot_statistical_comparison(
        model_results, fig_path_model,
        key_metrics_plot=key_metrics, metric_labels=labels, lower_is_better=lower,
    )
    print(f"Saved: {fig_path_model}")

    # Mix vs baselines (exclude model)
    mix_results = compare_vs_reference(
        df, reference="mix", baselines=("roll", "pers", "shrink"), metrics=metrics,
    )
    fig_path_mix = out_dir / "mix_vs_baselines.png"
    plot_statistical_comparison(
        mix_results, fig_path_mix,
        key_metrics_plot=key_metrics, metric_labels=labels, lower_is_better=lower,
    )
    print(f"Saved: {fig_path_mix}")
